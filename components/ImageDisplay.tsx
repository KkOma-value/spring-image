...
export const ImageDisplay: React.FC<ImageDisplayProps> = ({ imageUrl, isLoading, loadingMessage }) => {
    ...
    return (
      <div className="w-full aspect-square rounded-2xl glass-panel flex flex-col items-center justify-center p-8 text-center border-2 border-cny-gold/30">
        <div className="relative w-24 h-24 mb-6">
          <div className="absolute inset-0 border-4 border-cny-gold/20 rounded-full animate-spin"></div>
          <div className="absolute inset-0 border-t-4 border-cny-gold rounded-full animate-spin"></div>
  ...
  return (
    <div className="relative group w-full h-full">
      <div className={`relative w-full h-full overflow-hidden rounded-2xl shadow-2xl transition-all duration-500 border border-cny-gold/20 ${isZoomed ? 'fixed inset-0 z-50 bg-black/90 p-10 object-contain' : 'aspect-square'}`}>
         {isZoomed && (
             <button onClick={() => setIsZoomed(false)} className="absolute top-4 right-4 bg-white/10 p-2 rounded-full hover:bg-white/20">
         ...
         <img 
            ...
            className={`w-full h-full ${isZoomed ? 'object-contain' : 'object-cover'} transition-transform duration-700 hover:scale-105`}
            onClick={() => !isZoomed && setIsZoomed(true)}
         />

         {/* Overlay Actions (only visible when not zoomed or on hover when not zoomed) */}
         {!isZoomed && (
             <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex justify-between items-center">
                <span className="text-xs text-gray-300 font-mono">Gemini 2.5 Flash</span>
